...
fn hash_words(lmap:&mut LetterMap, letter:&String, buff:&String) {
    // splits the word buffer by white spaces 
    // the method also creates a iterator
    let mut letter_struct = lmap;
...
fn calculate_word_count(buff:String)->Vec<LetterMap>{
    ...
    for c in alphabet{
        thread::scope(|s| { //make a new thread for every letter of the alphabet
            let handle = s.spawn(|_| {
                hash_words(&mut hash_vec[i], &c.to_string(), &buff_copy);
            });
        }); 
        i+=1;
    }
